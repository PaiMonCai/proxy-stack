...
> Что реально тестируется: каждый коммит прогоняет полный набор тестов в контейнерах на тестовом VPS для **Debian 13, Ubuntu 24.04 / 22.04, Alpine 3.22, Rocky Linux 9 и AlmaLinux 8**. Остальные дистрибутивы из таблицы используют тот же код, но по отдельности не тестируются. SELinux в семействе Red Hat не тестировался (в контейнере его не включить). systemd в Amazon Linux 2 (219) слишком старый, чтобы запускать ядра без привилегий, поэтому там они работают от root (`psm doctor` объясняет причину). В EL8 PSM переключает Nginx на более новый модульный поток (1.24): штатный 1.14 не умеет SNI-маршрутизацию, нужную общему 443.
