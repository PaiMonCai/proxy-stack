...
Protocol inputs:
  ...
  VLESS Encryption (post-quantum): --vless-enc x25519|mlkem768 on xray reality /
...
_node_cli_defaults() {
    ...
    # VLESS Encryption：--vless-enc x25519|mlkem768 让对应内核生成一对密钥
    # （服务端 decryption / 客户端 encryption）。sing-box 的 VLESS 没有这个功能。
    local _venc _vpair
    _venc=$(printf '%s' "$json" | jq -r '.vless_enc // empty')
    if [[ -n "$_venc" && -z "$(printf '%s' "$json" | jq -r '.vless_decryption // empty')" ]]; then
        [[ "$_venc" == "x25519" || "$_venc" == "mlkem768" ]] \
            || { _node_cli_err "--vless-enc must be x25519 or mlkem768"; return 1; }
        case "$core/$proto" in
            xray/reality|xray/vision|xray/xhttp)
                declare -f xray_vlessenc_gen &>/dev/null || source "$LIB_DIR/xray/core.sh"
                _vpair=$(xray_vlessenc_gen "$_venc") ;;
            mihomo/vless)
                declare -f mh_vlessenc_gen &>/dev/null || source "$LIB_DIR/mihomo/vless.sh"
                _vpair=$(mh_vlessenc_gen "$_venc") ;;
            *)
                _node_cli_err "--vless-enc is only available for xray reality/vision/xhttp and mihomo vless"
                return 1 ;;
        esac
        [[ -n "$_vpair" ]] || { _node_cli_err "could not generate VLESS Encryption keys; is a recent $core installed?"; return 1; }
        json=$(printf '%s' "$json" | jq -c --arg d "${_vpair%%$'\t'*}" --arg e "${_vpair#*$'\t'}" \
            '.vless_decryption = $d | .vless_encryption = $e') || return 1
    fi
...
_node_cli_validate() {
    ...
    if ! printf '%s' "$json" | jq -e --arg proto "$proto" --arg core "$core" '
      if $proto == "reality" then
        ...
        ((.obfs_type // "salamander") as $o | ["salamander","gecko"] | index($o)) != null
