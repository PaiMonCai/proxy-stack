...
> 실제 테스트 범위: 모든 커밋은 테스트 VPS 의 컨테이너에서 **Debian 13, Ubuntu 24.04 / 22.04, Alpine 3.22, Rocky Linux 9, AlmaLinux 8** 에 대해 전체 테스트를 실행합니다. 표의 나머지 배포판은 같은 코드를 쓰지만 개별적으로 테스트하지는 않습니다. Red Hat 계열의 SELinux 는 테스트하지 않았습니다(컨테이너에서 켤 수 없음). Amazon Linux 2 의 systemd(219)는 너무 오래되어 코어를 비 root 로 시작할 수 없으므로 root 로 실행됩니다(`psm doctor` 가 이유를 알려 줍니다). EL8 에서는 기본 Nginx 1.14 에 공유 443 의 SNI 분기 기능이 없어 PSM 이 더 새로운 모듈 스트림(1.24)으로 전환합니다.
